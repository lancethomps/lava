...
public class RequestFactoryTest extends BaseTest {
	...
	/**
	 * Creates the from path key params test.
	 *
	...
	public void createFromPathKeyParamsTest() throws Exception {
		...
		final Map<String, Object> requestBean = RequestFactory.createFromPathKeyParams(
			MapUtil.createFromQueryString("data.name=test&data.type=PREFERENCE&data.children[0].name=testChild&data.children[0].type=PREFERENCE_CHILD").entrySet(),
			Serializer.MAP_TYPE,
			"data."
		);
	...
	/**
	 * Creates the from requests parameter.
	 *
	 * @throws Exception the exception
	 */
	@Test
	public void createFromRequestsParameter() throws Exception {
		String json = IOUtils.toString(RequestFactoryTest.class.getResourceAsStream("/request-factory/requests-parameter-test-data.json"), StandardCharsets.UTF_8);
		MockHttpServletRequest request = new MockHttpServletRequest();
		request.addParameter(
			"requests",
			json
		);
		List<MockRequestWithAllowedJsonParam> parsedFromRequest = RequestFactory.createBeansFromRequest(null, MockRequestWithAllowedJsonParam.class, request, true, true);
		...
		TestingCommon.assertEqualsViaJsonDiff("Requests parsed from allowed HTTP 'requests' parameter did not match requests deserialized from JSON.", parsedFromJson, parsedFromRequest);
	}
