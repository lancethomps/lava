...
import static com.lancethomps.lava.common.expr.ExprFactory.evalOgnl;
  ...
  public static <T extends Object> T mergeIntoResult(MergeConfig config, Map<String, Object> data, Map<String, Object> current, Class<T> resultType) {
    ...
    config.getFieldExpressions().stream().filter(field -> {
      return (field.getEnabled() == null) || field.getEnabled().stream().anyMatch(
        expr -> Lambdas.functionIfNonNull(evalOgnl(data, expr, false), res -> (res instanceof Boolean) && ((Boolean) res)).orElse(false)
      );
    }).forEach(field -> {
      Object val = field
        .getValue()
        ...
        .findFirst()
        .map(v -> Lambdas.functionIfNonNull(field.getPostProcess(), pp -> evalOgnl(v, pp, false)).orElse(null))
        .orElse(null);
