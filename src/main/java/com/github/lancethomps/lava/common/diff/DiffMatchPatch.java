...
public class DiffMatchPatch {
	...
	/** The Constant WORD_SEPARATOR_REGEX. */
	...
	/** The Constant WORD_SEPARATOR_PATTERN. */
	...
	public static String unescapeForEncodeUriCompatability(final String str) {
		return str
			.replace("%21", "!").replace("%7E", "~").replace("%27", "'").replace("%28", "(").replace("%29", ")").replace("%3B", ";").replace("%2F", "/").replace("%3F", "?").replace("%3A",
	 ...
	 * @return true, if is word separtor
	 */
	...
	public void diffCleanupEfficiency(final List<Diff> diffs) {
		...
		while (thisDiff != null) {
			if (thisDiff.getOperation() == DiffOperation.EQUAL) {
				 ...
				 */
				if ((lastequality != null) && ((preIns && preDel && postIns && postDel) || ((lastequality.length() < (diffEditCost / 2)) && (((preIns ? 1 : 0) + (preDel ? 1 : 0) + (postIns ? 1
					: 0) + (postDel ? 1 : 0)) == 3)))) {
	 ...
	 * @param text1 Old string to be diffed.
	 * @param text2 New string to be diffed.
	 * @param checklines Speedup flag. If false, then don't run a line-level diff first to identify the
	...
	/**
	 * Gets the patch delete threshold.
	 *
						...
						for (Diff aDiff : aPatch.getDiffs()) {
							if (aDiff.getOperation() != DiffOperation.EQUAL) {
								...
								if (aDiff.getOperation() == DiffOperation.INSERT) {
									...
									// Deletion
									resultText = resultText.substring(0, startLoc + index2) + resultText.substring(startLoc + diffXIndex(diffs, index1 + aDiff.getText().length()));
								}
							}
							if (aDiff.getOperation() != DiffOperation.DELETE) {
	...
	/**
	 * Compute a list of patches to turn text1 into text2. text2 is ignored, diffs are the delta between
	 * text1 and text2.
	 *
	 * @param text1 Old text
	 * @param text2 Ignored.
			...
			while (!bigPatchDiffs.isEmpty()) {
				...
				while (!bigPatchDiffs.isEmpty() && (patch.getLength1() < (patchSize - patchMargin))) {
					...
					if (diffType == DiffOperation.INSERT) {
						...
						empty = false;
					} else if ((diffType == DiffOperation.DELETE) && (patchDiffs.size() == 1) && (patchDiffs.getFirst().getOperation() == DiffOperation.EQUAL) && (diffText.length() > (2
						* patchSize))) {
	 ...
	 *         prefix of text2, the suffix of text2 and the common middle. Or empty string array there
	 *         was no match.
	 */
	 ...
	 * @param text1 First string.
	 * @param text2 Second string.
	 * @return An object containing the encoded text1, the encoded text2 and the List of unique strings.
	 *         The zeroth element of the List of unique strings is intentionally blank.
	 */
	...
	/**
	 * Locate the best instance of 'pattern' in 'text' near 'loc' using the Bitap algorithm. Returns -1
	 * if no match found.
	 *
	 * @param text The text to search.
	 * @param pattern The pattern to search for.
	 ...
	 *         prefix of shorttext, the suffix of shorttext and the common middle. or empty string array
	 *         there was no match.
	 */
	...
	/**
	 * Find overlaps between deletion and insertion.
	 *
	...
	/**
	 * Look for single edits on both sides.
	 *
	 * @param diffs the diffs
	...
	public enum DiffOperation {
		...
		/**
		 * Instantiates a new diff operation.
		 */
		DiffOperation() {
			this(null);
		}
	...
	/**
	 * Internal class for returning results from diffLinesToChars(). Other less paranoid languages just
	...
	protected static class LinesOrWordsToCharsResult {
		...
		/**
		 * Instantiates a new lines or words to chars result.
		 *
