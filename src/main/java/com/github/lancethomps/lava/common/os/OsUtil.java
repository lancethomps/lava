...
import static com.github.lancethomps.lava.common.lambda.Lambdas.functionIfTrue;
...
public class OsUtil {
	...
	/** The Constant GIT_COMMIT_COMMAND. */
	...
	/** The Constant WINDOWS_PROGRAM_FILES. */
	...
	public static boolean doesBinaryExist(final String binary) {
		try {
			return functionIfTrue(runAndGetOutput(asList("which", binary), false), StringUtils::isNotBlank, binaryPath -> new File(trim(binaryPath)).isFile(), bp -> false).orElse(false);
		} catch (IOException e) {
	...
	/**
	 * Gets the git head commit.
	 *
	 * @param gitRepo the git repo
	 * @return the git head commit
	 */
	 ...
	 *
	 * @param gitRepo the git repo
	 * @param tryGitBinary the try git binary
	 * @return the git head commit
	 */
	...
	/**
	 * Gets the process output from file.
	 *
	 * @param process the process
	 * @param file the file
	 * @return the process output from file
	 */
	...
	/**
	 * Gets the quoted command.
	 *
	 * @param commands the commands
	 * @return the quoted command
	 */
	...
	/**
	 * Gets the shares root.
	 *
	 * @return the shares root
	 */
	...
	/**
	 * Launch windows default browser.
	 *
	 ...
	 *
	 * @param commands the commands
	 * @param redirectError the redirect error
	 * @return the process
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	...
	private static boolean searchDefaultPath(String targetUrl) {
		...
		FilenameFilter chromeDirfilter = new FilenameFilter() {
			...
			public boolean accept(File dir, String name) {
				if (name.toLowerCase().indexOf("chrome") > 0) {
					return true;
				}
				return false;
			}
